...
void main_task(intptr_t unused) {
	...
	int ercd = act_tsk(SERIAL_TASK);
  
    syslog(LOG_NOTICE, "#### motor control start %d", ercd);
    int count = 0;
...
void serial_task(intptr_t unused) {
	syslog(LOG_EMERG, "####serial_task UP");
	while (1)
    {
    	int rlen;
    	static char buffer[1024];
	    rlen = athrill_serial_readline(0, buffer, sizeof(buffer));
    	if (rlen > 0) {
	    	syslog(LOG_EMERG, "%s", buffer);
    	}
    	athrill_serial_send(0, "Hello World from athrill");
        tslp_tsk(100000); /* 100msec */
    }
}
